...
```bash
conda create -n ocr_eval_all_paddlevl python=3.10.20 -y
conda activate ocr_eval_all_paddlevl
...
```bash
conda env export -n ocr_eval_all_paddlevl > envs/ocr_eval_all_paddlevl.lock.yml
pip freeze > envs/ocr_eval_all_paddlevl.freeze.txt
```

GPU 환경(권장: 전용 env 분리)에서는 `paddlepaddle`/`paddlepaddle-gpu`를 같은 env에 같이 설치하지 않습니다.

```bash
conda create -n ocr_eval_all_paddlevl_gpu python=3.10.20 -y
conda activate ocr_eval_all_paddlevl_gpu
pip install -r requirements_ocr_eval_vlm_gpu.txt

# CUDA 버전에 맞는 Paddle 공식 wheel 인덱스로 GPU 런타임 설치 (둘 중 하나)
# CUDA 11.8 예시
