...
│   ├── v1/                  # OpenAI 시나리오 데이터/산출물
│   └── v2/                  # Hugging Face 시나리오 데이터/산출물
...
```bash
# 기본 설정은 data/v1/eval_questions.example.jsonl 을 사용합니다.
...
python -m src.cli parse-hwp \
  --input "data/raw/<input>.hwp" \
  --output "data/v1/<name>_prechunk.jsonl" \
  --debug-headings "data/v1/<name>_heading_debug.jsonl"

python -m src.cli chunk-jsonl \
  --input "data/v1/<name>_prechunk.jsonl" \
  --output "data/v1/<name>_chunks.jsonl"

python -m src.cli embed-jsonl \
  --input "data/v1/<name>_chunks.jsonl" \
  --output "data/v1/<name>_embedded.jsonl" \
  --model "text-embedding-3-small"

python -m src.cli build-chroma \
  --input "data/v1/<name>_embedded.jsonl" \
